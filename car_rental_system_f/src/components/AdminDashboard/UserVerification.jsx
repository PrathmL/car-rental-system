...
const UserVerification = () => {
  ...
  const fetchRequests = async () => {
    try {
      const response = await axios.get('http://localhost:8080/api/admin/pending/customer');
  ...
  const handleAction = async (userId, action) => {
    try {
      ...
      showAlert('Processed', `Customer ${action === 'approve' ? 'Approved' : 'Rejected'}!`, 'success');
      fetchRequests();
    } catch (error) {
        ...
        <tbody>
          {requests.length > 0 ? requests.map(user => (
            <tr key={user.id}>
              ...
              <td>
                <a href={`http://localhost:8080${user.documentUrl}`} target="_blank" rel="noreferrer" className="btn btn-sm btn-light border rounded-pill px-3 fw-bold">
            ...
            <tr><td colSpan="4" className="text-center py-5 text-muted bg-white rounded-4">No pending customer verifications found.</td></tr>
          )}
        </tbody>
      </table>
    </div>
